...
class Solution:
    def minimumCardPickup(self, cards: list[int]) -> int:
        ...
        for i in range(len(cards)):
            if cards[i] in dct:
                answer = min(answer, i - dct[cards[i]] + 1)
...
# Space complexity: O(M), where M is the number of unique card values
# in the cards array. In the worst case, we may store all card values
# in the dictionary.
